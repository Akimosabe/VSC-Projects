...
class python_snake:
    def __init__(self, window, canv_x, canv_y, canv_width, canv_height):
        ...
        self.__window.bind(
            "<KP_Subtract>", self.speed_key
        )  # Клавиша - на боковой клаве
    ...
    class CONST(Enum):  # Список возможных направлений движения и других констант
        ...
        EXPLOSIVE = (
            15  # Диаметр взрыва при столкновении змеи с препятствием (нечётное число)
        )
        EXPLOSIVE_BORD = (
            10  # толщина контура взрыва при столкновении змеи с препятствием
        )
    ...
    def create_head_food(self):
        ...
        self.body = []
        self.body.append(
            {"id": self.head.draw(), "x": self.__snake_x, "y": self.__snake_y}
        )
        self.step("add")
        self.step("add")
        self.step("add")
    ...
    def bump_wall(self):  # Проверка на столкновение со стеной
        ...
        if (
            (__head_x < ((self.CONST.SNAKE_THICKNESS.value // 2) + 1))
            or (__head_y < ((self.CONST.SNAKE_THICKNESS.value // 2) + 1))
            or (
                __head_x
                > (self.canv_width - (self.CONST.SNAKE_THICKNESS.value // 2) + 1)
            )
            or (
                __head_y
                > (self.canv_height - (self.CONST.SNAKE_THICKNESS.value // 2) + 1)
            )
        ):
            self.explosive()
            return "the end"
    ...
    def step(self, add):  # Двигать тело змеюки в текущую сторону на 1 шаг
        ...
        self.body.append(
            {"id": self.head.draw(), "x": self.head.x, "y": self.head.y}
        )  # Создал новую голову
        self.canv.itemconfig(
            self.body[-2]["id"], fill=self.CONST.SNAKE_BCOLOR.value
        )  # Перекрасил старую голову в тело
        if add != "add":
    ...
    class food:
        ...
        def eat(self):
            ...
            if (
                ...
                and (
                    head_y + self.CONST.SNAKE_THICKNESS.value // 2
    ...
    class element_square:  # Рисую квадратик со стороной d и центром x,y
        ...
        def draw(self):
            ...
            return self.self_glob.canv.create_rectangle(
                x, y, x + self.d, y + self.d, fill=self.color, width=2
            )
...
def main():
    ...
    text = Label(
        root,
        text="""Правила: Змейка должна кушать
